...
export const options = {
  vus: 5,           // adjust as needed
  duration: '20s',  // adjust as needed
  thresholds: {
...
export default function () {
  ...
  // 🔹 GET /applications/read/my-applications
  let res = http.get(
    'https://recruitai-backend-production.up.railway.app/v1/applications/read/my-applications',
  ...
  if (res.status === 419) {
    ...
    res = http.get(
      'https://recruitai-backend-production.up.railway.app/v1/notifications/read/get-all',
      { headers: { Authorization: `Bearer ${access_token}` } }
    );
  }

  check(res, { 'get my applications status is 200': (r) => r.status === 200 });

  if (res.status !== 200) {
    console.log(`FAILED GET MY APPLICATIONS: Status=${res.status}, Response=${res.body}`);
  }
